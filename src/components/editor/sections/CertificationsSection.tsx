    ...
    <div className="space-y-5">
      <div className="flex items-baseline justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-zinc-500">Add relevant credentials (optional).</div>
        </div>
        <button
          type="button"
      ...
      <DndContext
        ...
        onDragEnd={(event) => {
          ...
          useCVStore.getState().reorderEntries('certifications', from, to);
        }}
      >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      >
                        <div className="truncate text-sm font-semibold text-zinc-900">
                          ...
                          {c.date?.trim() ? (
                            <span className="font-normal text-zinc-500">— {c.date}</span>
                          ) : null}
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => removeEntry('certifications', c.id)}
                    ...
                    <div className="mt-3 space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <Field
                          label="Name"
                          value={c.name}
                          onChange={(v) => updateEntry('certifications', c.id, { name: v })}
                        />
                        <Field
                          label="Issuer"
                          value={c.issuer}
                          onChange={(v) => updateEntry('certifications', c.id, { issuer: v })}
                        />
                        <Field
                          label="Date"
                          value={c.date}
                          onChange={(v) => updateEntry('certifications', c.id, { date: v })}
                      ...
                      <div>
                        ...
                        <RichTextField
                          ...
                          onChange={(html) =>
                            updateEntry('certifications', c.id, { descriptionHtml: html })
                          }
                        />
                      </div>
                    </div>
